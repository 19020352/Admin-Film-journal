      ...
      <template v-slot:item.Action="{ item }">
        <v-menu offset-y>
          ...
          <v-list color="#e4e9f0" dense>
            <v-list-item link @click="handleOnEdit(item.Similar_filmID)">
              <v-list-item-content>
                <v-list-item-title>Edit</v-list-item-title>
              </v-list-item-content>
            </v-list-item>
            <v-list-item link @click="handleOnDelete(item.Similar_filmID)">
              <v-list-item-content>
...
export default {
  data() {
    return {
      total: 500,
      listHeadData: [
        { value: "Similar_filmID", text: "Similar_film ID", align: "center" },
